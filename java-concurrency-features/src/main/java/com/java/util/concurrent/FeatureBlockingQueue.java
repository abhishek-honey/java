...
public class FeatureBlockingQueue {
	private static BlockingQueue<Long> blockingQeque = new ArrayBlockingQueue<>(1);
	...
	private static void usingCallable() {
		Callable<Boolean> offerCallable = () -> {
			return blockingQeque.offer(atomicLong.getAndIncrement());
		};

		Callable<Long> pollCallable = blockingQeque::poll;
	...
	private static void usingRunnable() {
		Runnable offerRunnable = () -> System.out
				.println(Thread.currentThread().getName() + "\ttimestamp:\t" + System.nanoTime() + " " + " Offering: "
						+ blockingQeque.offer(atomicLong.getAndIncrement()) + " " + atomicLong);

		Runnable pollRunnable = () -> System.out.println(Thread.currentThread().getName() + "\t timestamp:\t"
				+ System.nanoTime() + " " + " Polling: " + blockingQeque.poll());
